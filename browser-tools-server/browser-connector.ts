...
export class BrowserConnector {
  ...
  async authTokenProxy(
    ...
    if (!origin || !storageType || !tokenKey) {
      res.status(400).json({
        error:
          "Missing required parameters. Please provide origin, storageType, and tokenKey.",
      });
      return;
    }
    ...
    if (!["cookie", "localStorage", "sessionStorage"].includes(storageType)) {
      res.status(400).json({
        error:
          "Invalid storageType. Must be 'cookie', 'localStorage', or 'sessionStorage'.",
      });
      return;
    }
    ...
    try {
      ...
      const messagePromise = new Promise<any>((resolve, reject) => {
        ...
        setTimeout(() => {
          this.activeConnection?.removeListener("message", messageHandler);
          reject(
            new Error("Timeout waiting for response from browser extension")
          );
  ...
  async authenticatedApiCall(
    ...
    if (!authConfig || !apiCall) {
      res.status(400).json({
        error:
          "Missing required parameters. Please provide authConfig and apiCall objects.",
      });
      return;
    }
    ...
    try {
      ...
      const tokenPromise = new Promise<string>((resolve, reject) => {
        ...
        setTimeout(() => {
          this.activeConnection?.removeListener("message", messageHandler);
          reject(
            new Error("Timeout waiting for auth token from browser extension")
          );
        }, 10000);
      });
      ...
      console.log(
        `Step 2 Complete: API call completed with status ${apiResponse.status}`
      );
