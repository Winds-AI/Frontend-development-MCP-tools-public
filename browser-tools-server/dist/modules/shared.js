...
let cachedProjectsConfigMtimeMs = null;
...
function resolveProjectsJsonPath() {
    ...
    if (!fs.existsSync(rootPath)) {
        throw new Error("projects.json not found at project root. Open the Setup UI to create and save it.");
    }
    return rootPath;
}
export function loadProjectConfig() {
    try {
        ...
        const shouldReload = !cachedProjectsConfig ||
        ...
        if (shouldReload) {
            ...
            if (previousPath && previousPath !== configPath) {
                console.log(`[INFO] Browser Connector: Reloaded projects.json from ${configPath} (was ${previousPath})`);
            }
            if (!hasLoggedProjectsConfig) {
