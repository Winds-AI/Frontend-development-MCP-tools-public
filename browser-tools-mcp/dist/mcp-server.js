...
function generateNavigateToolDescription() {
...
async function resolveAuthToken(includeAuthToken) {
    ...
    if (!storageType || !tokenKey) {
        return {
            error: "Auth token retrieval not configured. Set AUTH_STORAGE_TYPE, AUTH_TOKEN_KEY, and optional AUTH_ORIGIN in projects.json.",
        };
    }
    const token = await retrieveTokenViaExtension();
    if (!token) {
        return {
            error: "Failed to retrieve auth token from configured browser storage. Ensure the target app is open and DevTools extension connected.",
        };
    }
...
async function withServerConnection(apiCall) {
    ...
    try {
        ...
        console.error(`API call failed: ${error.message}. Attempting rediscovery...`);
...
server.tool("browser.console.read", "Read browser console logs with filters; returns formatted summary + stats. Captures JS errors/warnings/logs and browser-generated network errors (e.g., 'Failed to load resource'). For full HTTP payloads and headers, use 'browser.network.inspect'.", {
    level: z
